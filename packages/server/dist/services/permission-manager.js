...
export class PermissionManager extends EventEmitter {
    ...
    async checkMacOSBluetoothPermissions() {
        try {
            ...
            try {
                ...
                const { stdout: permissions } = await execAsync(`sqlite3 ~/Library/Application\\ Support/com.apple.TCC/TCC.db "SELECT * FROM access WHERE service='kTCCServiceBluetoothAlways' AND client LIKE '%${appName}%'" 2>/dev/null || echo "no_permission_entry"`);
    ...
    async checkLinuxBluetoothPermissions() {
        try {
    ...
    async checkWindowsBluetoothPermissions() {
        try {
    ...
    async checkMacOSUSBPermissions() {
        try {
            ...
            const hasGarminANT = usbDevices.includes('Garmin') && (usbDevices.includes('ANT') ||
    ...
    async checkLinuxUSBPermissions() {
        try {
            // Check for ANT+ USB devices using lsusb
            const { stdout: usbDevices } = await execAsync('lsusb');
            // Look for Garmin ANT+ devices
            const hasGarminANT = usbDevices.includes('0fcf:') && (usbDevices.includes('1008') || usbDevices.includes('1009'));
            if (hasGarminANT) {
    ...
    async checkWindowsUSBPermissions() {
        try {
