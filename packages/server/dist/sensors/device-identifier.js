...
const OFFICIAL_SERVICE_DESCRIPTIONS = {
    '180d': 'Heart Rate',
    '1818': 'Cycling Power',
...
const DEVICE_NAME_PATTERNS = {
    // Heart Rate Monitors
    'HR': { type: 'heart_rate', category: 'Heart Rate Monitor' },
    'Heart': { type: 'heart_rate', category: 'Heart Rate Monitor' },
    'Polar': { type: 'heart_rate', category: 'Heart Rate Monitor', manufacturer: 'Polar' },
    'Wahoo': { type: 'heart_rate', category: 'Multi-sensor Device', manufacturer: 'Wahoo' },
    'Garmin': { type: 'heart_rate', category: 'Multi-sensor Device', manufacturer: 'Garmin' },
    'Suunto': { type: 'heart_rate', category: 'Heart Rate Monitor', manufacturer: 'Suunto' },
    // Power Meters
    'Power': { type: 'power', category: 'Power Meter' },
    'Stages': { type: 'power', category: 'Power Meter', manufacturer: 'Stages' },
    'Quarq': { type: 'power', category: 'Power Meter', manufacturer: 'Quarq' },
    'SRAM': { type: 'power', category: 'Power Meter', manufacturer: 'SRAM' },
    'Rotor': { type: 'power', category: 'Power Meter', manufacturer: 'Rotor' },
    '4iiii': { type: 'power', category: 'Power Meter', manufacturer: '4iiii' },
    'PowerTap': { type: 'power', category: 'Power Meter', manufacturer: 'PowerTap' },
    // Speed/Cadence Sensors  
    'Speed': { type: 'cadence', category: 'Speed/Cadence Sensor' },
    'Cadence': { type: 'cadence', category: 'Speed/Cadence Sensor' },
    'CSC': { type: 'cadence', category: 'Speed/Cadence Sensor' },
    'DuoTrap': { type: 'cadence', category: 'Speed/Cadence Sensor', manufacturer: 'Bontrager' },
    // Smart Trainers
    'KICKR': { type: 'trainer', category: 'Smart Trainer', manufacturer: 'Wahoo' },
    'Neo': { type: 'trainer', category: 'Smart Trainer', manufacturer: 'Tacx' },
    'Flux': { type: 'trainer', category: 'Smart Trainer', manufacturer: 'Tacx' },
    'Elite': { type: 'trainer', category: 'Smart Trainer', manufacturer: 'Elite' },
    'Trainer': { type: 'trainer', category: 'Smart Trainer' },
    // Other Known Brands
    'Magene': { type: 'cadence', category: 'Multi-sensor Device', manufacturer: 'Magene' },
    'Bryton': { type: 'cadence', category: 'Bike Computer', manufacturer: 'Bryton' },
    'Lezyne': { type: 'cadence', category: 'Multi-sensor Device', manufacturer: 'Lezyne' },
    'CatEye': { type: 'cadence', category: 'Bike Computer', manufacturer: 'CatEye' }
};
...
export class DeviceIdentifier {
     ...
     */
    static identifyDevice(peripheral, services, characteristics) {
     ...
     */
    static parseManufacturerData(manufacturerData) {
     ...
     */
    static analyzeDeviceName(localName) {
        if (!localName)
            return null;
     ...
     */
    static analyzeServices(serviceUuids) {
        const services = [];
        const capabilities = [];
        let confidence = 0;
        let primaryType;
        for (const uuid of serviceUuids) {
            ...
            const description = SERVICE_DESCRIPTIONS[normalizedUuid];
            services.push({
     ...
     */
    static analyzeCharacteristics(characteristics) {
        const charInfo = [];
        const capabilities = [];
        let confidence = 0;
        for (const char of characteristics) {
            const uuid = char.uuid.toLowerCase();
            const description = CHARACTERISTIC_DESCRIPTIONS[uuid];
            charInfo.push({
                uuid: uuid,
     ...
     */
    static generateDeviceName(identification, localName, deviceId) {
        const parts = [];
     ...
     */
    static generateDisplayName(identification, localName) {
        const parts = [];
     ...
     */
    static calculateNameConfidence(deviceName, pattern) {
        ...
        if (name.includes(pat))
            return 30;
        if (name.includes(pat.substring(0, 3)))
            return 20;
        return 10;
    }
    /**
     ...
     */
    static extractModelFromName(deviceName, pattern) {
     ...
     */
    static inferTypeFromCapabilities(capabilities) {
        if (capabilities.some(cap => cap.includes('Power')))
            return 'power';
        if (capabilities.some(cap => cap.includes('Heart Rate')))
            return 'heart_rate';
        if (capabilities.some(cap => cap.includes('Trainer')))
            return 'trainer';
        if (capabilities.some(cap => cap.includes('Speed') || cap.includes('Cadence')))
            return 'cadence';
